...
const experiences = [
  {
    ...
    points: [
      ...
      'Built an **AI-powered search system** using **LangChain, FAISS, and Mistral**, implementing **RAG architecture** to enable natural language queries across healthcare codebase, billing documentation, and claims datasets with **fully offline infrastructure**',
      'Transforming legacy **Jinja-based UI** into a **modern React design system** using **ShadCN and TypeScript**, migrating server-rendered templates to reusable component architecture for immunization tracking, mental health records, Medicaid reporting, and claims management with standardized validation and audit-ready workflows',
      'Built **EduClaim analytics dashboard** providing **real-time billing insights** for K-12 districts, featuring automated claims tracking, revenue cycle insights, and denial pattern analysis that **increased billing efficiency by 25%** and **recovered $1M+ in previously denied claims**',
      'Orchestrated **GitLab to GitHub migration**, implementing automated **CI/CD pipelines** with unit and integration test suites and **AWS deployments**, while enhancing **repository security** by removing exposed credentials and standardizing git workflows across the development team',
      'Integrated **New Relic Python agent** into Flask APIs with custom alerting workflows for performance monitoring, identifying memory leaks and implementing resource optimization that **reduced server costs by 40%**',
    ],
    tech_stack: [
      'React.js',
      'TypeScript',
  ...
  },
  {
    ...
    points: [
      'Enhanced data exploration for business users by building **conversational AI analytics platform** with React, Material-UI, Flask, Go, and SQLAlchemy, enabling **natural language query-to-visualization workflows** with **100+ domain-aware commands** that eliminated manual chart construction',
      'Launched **revenue-generating SaaS solution** by integrating **Stripe billing, Google SSO, and tiered subscription management**, transforming DataChat from an internal tool into a **commercial no-code analytics platform** serving enterprise customers',
      'Resolved **database integration bottlenecks** by implementing **on-demand connection architecture**, eliminating connection pool exhaustion and enabling efficient resource management across **Snowflake, BigQuery, Databricks, and 10+ other data sources**',
      'Developed **collaboration features** with **Algolia-powered search**, enabling **cross-organizational data sharing** and advanced filtering capabilities, transforming individual dashboards into team workspaces used by business users',
      'Engineered automated **Kubernetes infrastructure** using **Terraform with S3-backed remote state**, developing modular deployment configurations that **reduced customer environment provisioning from manual weeks to automated hours**',
      'Implemented **end-to-end testing framework** using **Cypress** for conversational AI workflows and **Flask/Go test suites** for API validation, ensuring accurate query processing, data visualization, and multi-source integration',
      'Maintained and optimized **distributed messaging infrastructure** using **RabbitMQ and AMQP protocols**, supporting async message handling and request processing for **real-time analytics workloads**',
    ],
    tech_stack: [
      'React.js',
      'TypeScript',
